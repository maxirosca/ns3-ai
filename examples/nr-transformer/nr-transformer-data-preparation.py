...
# Load the CSV files
input_df = pd.read_csv('/home/maximilianrosca/ns-3-dev/inputs_DlTransmission.csv')
output_df = pd.read_csv('/home/maximilianrosca/ns-3-dev/outputs_DlTransmission.csv')
...
# # Define the 12 OFDM symbol output columns (sym0–sym11)
# out_sym_cols = [f"sym{i}" for i in range(12)]
...
#     group_sorted = group.sort_values(by="rnti", kind="mergesort")

#     # Convert each flow’s features to a compact string: "rnti:resource_type:priority:..."
#     flow_strs = group_sorted[flow_cols].agg(":".join, axis=1).tolist()
...
# clean_input_path = "/home/maximilianrosca/ns-3-dev/inputs_DlTransmission_no_duplicates.csv"
# clean_output_path = "/home/maximilianrosca/ns-3-dev/outputs_DlTransmission_no_duplicates.csv"
...
for key, group in input_df.groupby(slot_cols):
    ...
    if num_flows < MAX_FLOWS:
        ...
        pad_rows = pd.concat(
            [pd.DataFrame([[*key, 0, 0, 0, 0, 0, 0, 0, 0]], columns=slot_cols + flow_cols)] * (MAX_FLOWS - num_flows),
            ignore_index=True
        )
...
cols_to_zscore = ["priority", "delay_budget", "hol_delay",
...
# Drop simulation, frame, subframe, and slot columns in the input and output DataFrames
padded_input_df = padded_input_df.drop(columns=['simulation', 'frame', 'subframe', 'slot'])
output_df = output_df.drop(columns=['simulation', 'frame', 'subframe', 'slot'])

# Save the processed DataFrame to a new CSV file
padded_input_df.to_csv('/home/maximilianrosca/ns-3-dev/inputs_DlTransmission_zscore.csv', index=False)
output_df.to_csv('/home/maximilianrosca/ns-3-dev/outputs_DlTransmission_zscore.csv', index=False)
